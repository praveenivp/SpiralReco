%%
  parameter=struct('Ninterleaves', 1,'Resolution',1,'FOV',[1 1 1 1].*200,'MaxGradAmp',4,...
      'MaxSlewrate',20,'DwellTime',2e-6,'B0effects',2,'T2star',100e-3);%fov,resolution
a=MriSim( parameter);
...
toc
imc_MTI=MTIOP'*a.data(:);
toc
as(imc_MTI)
%%
figure,
subplot(221),imagesc([],[],real(im),[0 180]),title('Uncorrected image'),axis square,colorbar
subplot(222),imagesc(a.b0_map),title('B0 map(Hz)'),axis square,colorbar
subplot(223),imagesc([],[],abs(imc_MFI),[0 180]),title('MFI correction(30 sec)'),axis square,colorbar
subplot(224),imagesc([],[],abs(imc_MTI).*1.3,[0 180]),title('MTI correction(6 sec)'),axis square,colorbar
...
f(:,1)=f0(:);
for i=1:50
r=(y-Fope*reshape(f(:,i),FT.imSize(1),FT.imSize(2))); % residual
% figure(11),plot(real(r)),hold on
g_new=col(Fope.*r-beta*getRoughness(f(:,i),FT.imSize));
...
if(i==1)
    gamma=0;
    d=M.*g_new;
    q=Fope*reshape(d,FT.imSize(1),[]);
    alpha=d'*g_new/(q'*q) *4e4;
else
    gamma=(g_new'*(M.*g_new))/(g_old'*(M.*g_old));
    d=M.*g_new+gamma*d;
%     d=d.*1e3;
    q=Fope*reshape(d,FT.imSize(1),[]);
    alpha=d'*g_new/(q'*q+beta*d'*col(getRoughness(d,FT.imSize)))*4e4;
end
...
%%
figure,
subplot(231),imagesc(abs(im)),title('Uncorrected image'),axis square,colorbar
subplot(232),imagesc(a.b0_map),title('B0 map(Hz)'),axis square,colorbar
subplot(233),imagesc(abs(imc_MFI)),title('MFI correction(30 sec)'),axis square,colorbar
subplot(234),imagesc(abs(imc_MTI)),title('MTI correction(6 sec)'),axis square,colorbar
subplot(235),imagesc(flip(flip(abs(reshape(im_pcg,FT.imSize)),2),1)),title('PCG exact'),axis square,colorbar
subplot(236),imagesc(flip(flip(abs(reshape(f(:,end),FT.imSize)),2),1)),title('Iterative exact'),axis square,colorbar
