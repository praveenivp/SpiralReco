...
# running FSL script
Before running FSL FEAT, we need the stimulus file from the log generated by pyschopy using the follwing script.
...
```
Run a sample FEAT analysis, copy the design.fsf as `template.fsf` and copy the script into the folder.

```bash
runFSL.sh <moco_nifiti_volume> <template.fsf> <paradigm file> <smoothfac>
```
