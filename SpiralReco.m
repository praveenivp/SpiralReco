...
%             posi=1e-3*obj.SpiralPara.slice{1}.Position(idx); %m
        ...
        function performRecon(obj)
            ...
            if(obj.flags.is3D)
                ...
                for cha=1:size(obj.sig,1)
                    obj.img(cha,:,:,1,obj.LoopCounter.cSlc,obj.LoopCounter.cRep) = obj.NUFFT_obj'*(obj.sig(cha,:));
                end
            end
        ...
        function performRecon3D(obj)
            
            for par=1:obj.twix.image.NPar
                for cha=1:size(obj.sig,1)
                    obj.img(cha,:,:,par,obj.LoopCounter.cSlc,obj.LoopCounter.cRep) = obj.NUFFT_obj'*(obj.sig(cha,:,par));
                end
            end
